...
fn post_upgrade(auth: Option<UpgradeAuth>) {
    ...
    if let Some(auth_info) = auth {
        ...
        let version = auth_info.version.clone();
        ...
        log_event(
            EventType::AdminTransferCompleted,
            format!("Canister upgraded to version {}", version)
        );
    } else {
...
fn create_policy(premium: Nat, coverage: Nat) -> Result<PolicyId, String> {
    ...
    // Log event after releasing the mutable borrow
...
fn set_flood_level(flood_level: i64) -> Result<(), String> {
    ...
    STATE.with(|cell| {
        ...
        let result = (|| {
            ...
            // Check for anomalous changes (store for later logging)
            if state.oracle_update_count > 0 {
                let change = (flood_level - state.flood_level).abs();
                if change > ANOMALY_THRESHOLD {
                    anomaly_detected = Some(format!("Large flood level change detected: {} by {}", 
    ...
    // Log events after releasing the borrow
    if let Some(anomaly_msg) = anomaly_detected {
        log_event(EventType::AnomalyDetected, anomaly_msg);
    }
...
fn trigger_payout() -> Result<Nat, String> {
    ...
    let result = STATE.with(|cell| {
        ...
        let result = (|| {
            ...
            let policy_id = *policy_id; // Copy the ID to avoid borrow issues
...
fn complete_admin_transfer() -> Result<(), String> {
    ...
    let result = STATE.with(|state| {
        ...
        Ok((old_admin, pending))
    });
    
    // Log event after releasing the mutable borrow
    if let Ok((old_admin, new_admin)) = result {
        log_event(
            EventType::AdminTransferCompleted,
            format!("Admin transferred from {} to {}", 
                old_admin.to_text(), new_admin.to_text())
        );
        Ok(())
    } else {
        result.map(|_| ())
...
fn add_oracle_updater(oracle: Principal) -> Result<(), String> {
    ...
    let result = STATE.with(|state| {
        ...
        if !state.oracle_updaters.contains(&oracle) {
            ...
            return Ok(true); // Added successfully
        }
        
        Ok(false) // Already exists
    });
    
    // Log outside the STATE borrow
    match &result {
        Ok(true) => {
            log_event(
...
fn remove_oracle_updater(oracle: Principal) -> Result<(), String> {
    ...
    let result = STATE.with(|state| {
        ...
        let old_len = state.oracle_updaters.len();
        state.oracle_updaters.retain(|&p| p != oracle);
        let removed = old_len != state.oracle_updaters.len();
...
fn set_flood_threshold(threshold: u64) -> Result<(), String> {
    ...
    let result = STATE.with(|state| {
        ...
        Ok((old_threshold, threshold))
    });
    
    // Log event after releasing the mutable borrow
    if let Ok((old_threshold, new_threshold)) = result {
        log_event(
            EventType::ThresholdChanged,
            format!("Flood threshold changed from {} to {}", old_threshold, new_threshold)
        );
        Ok(())
    } else {
        result.map(|_| ())
