...
import { getCurrentPrincipal, formatPrincipal, isAuthenticated, createPolicy, getUserPolicy, claimPayout } from "../lib/icp";
...
export default function IndividualDashboard({ setUserType }: IndividualDashboardProps) {
  ...
  const loadPolicies = async () => {
    try {
      ...
      if (userPolicy) {
        // Convert the ICP Policy format to our frontend format
        const policy: Policy = {
          id: userPolicy.policy_id.toString(),
          principal: userPolicy.policyholder.toText(),
          coverageAmount: Number(userPolicy.coverage) / 1000000, // Convert from units to millions
          premium: Number(userPolicy.premium) / 1000000, // Convert from units to millions
          isActive: userPolicy.active && !userPolicy.paid_out,
          createdAt: new Date(Number(userPolicy.purchase_time) / 1000000).toISOString(), // Convert from nanoseconds
          thresholdExceeded: false, // This would need to be calculated based on current flood level
          claimedAt: userPolicy.paid_out ? new Date().toISOString() : undefined
        };
        
        setPolicies([policy]);
        setActivePolicy(policy.isActive ? policy : null);
      } else {
        setPolicies([]);
  ...
  const handleBuyInsurance = async () => {
    try {
      ...
      if (result.success) {
        setWalletBalance(walletBalance - amount * 0.1); // Deduct premium (10% of coverage) from wallet
        setMessage(`✅ Insurance policy purchased! Coverage: $${coverageAmount}M, Premium: $${(amount * 0.1).toFixed(1)}M`);
        await loadPolicies(); // Reload policies
      } else {
        setMessage(`❌ Failed to purchase insurance: ${result.error || 'Unknown error'}`);
      }
    } catch (error) {
      console.error("Failed to buy insurance:", error);
      setMessage(`❌ Failed to purchase insurance: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleClaimPayout = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage(`❌ Failed to claim payout: ${result.error || 'Unknown error'}`);
      }
    } catch (error) {
